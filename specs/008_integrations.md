...
| Produces | Integration configuration properties, RestClient infrastructure, Resilience4j setup, health configuration, logging interceptor |
| Context | Defines the integration patterns and infrastructure for external HTTP services. Messaging/event streaming is intentionally out of scope for v1. The external services (Schedules, Equipment, Quotes) are owned by other teams and their API contracts are NOT yet known. |
...
### IntegrationProperties configuration class
...
### RestClient bean factory pattern

Tags: `integration`, `pattern`
...
### How to implement a real client when an external API contract is available
...
### Items NOT covered in integrations

Tags: `integration`, `out-of-scope`
