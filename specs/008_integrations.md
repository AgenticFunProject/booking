...
Feature: Integration Infrastructure and Kafka Configuration
  ...
  I need to set up the integration infrastructure and Kafka producer configuration
  ...
  # ---------------------------------------------------------------------------
  # Kafka Producer Fine-Tuning
  # ---------------------------------------------------------------------------

  @integration @kafka
  Scenario: Kafka producer reliability settings
    Given the file "src/main/resources/application.yml"
    Then it must include the following Kafka producer settings beyond what 001 defines:
      | property                                            | value                | description                             |
      | spring.kafka.producer.acks                          | all                  | Wait for all replicas to acknowledge    |
      | spring.kafka.producer.retries                       | 3                    | Retry on transient failures             |
      | spring.kafka.producer.properties.enable.idempotence | true                 | Prevent duplicate messages              |
      | spring.kafka.producer.properties.max.in.flight.requests.per.connection | 1 | Preserve ordering with retries        |
      | spring.kafka.producer.properties.delivery.timeout.ms| 30000               | Max time for a send to complete         |

  @integration @kafka
  Scenario: Kafka error handling in BookingEventPublisher
    Given the BookingEventPublisher from 004_business_rules.md
    Then the publish methods must use KafkaTemplate.send() with a callback:
      | outcome  | action                                                                        |
      | success  | Log at INFO: "Event {type} published for booking {reference} to topic {topic}"|
      | failure  | Log at ERROR: "Failed to publish event {type} for booking {reference}: {error message}" |
    And publish failures must NOT throw exceptions or roll back the booking transaction
    And the KafkaTemplate must be configured with a ProducerListener or CompletableFuture callback
  ...
  Scenario: Actuator health endpoint configuration
    ...
    And the Kafka health indicator is already provided by Spring Boot Actuator (no custom code needed)
